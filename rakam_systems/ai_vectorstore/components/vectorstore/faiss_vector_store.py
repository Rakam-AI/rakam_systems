...
class FaissStore(VectorStore):
    ...
    def add(self, vectors: List[List[float]], metadatas: List[Dict[str, Any]]) -> Any:
        ...
        existing_ids = set(self.collections[collection_name]["category_index_mapping"].keys())
        ...
        for idx, (vec, meta) in enumerate(zip(vectors, metadatas)):
            ...
            self.collections[collection_name]["category_index_mapping"][node_id] = meta.get("content", "")
    ...
    def predict_embeddings(self, query: str) -> np.ndarray:
        ...
        if self.use_embedding_api:
            query_embedding = (
                self.client.embeddings.create(input=[query], model=self.api_model)
                .data[0]
                .embedding
            )
    ...
    def add_nodes(self, collection_name: str, nodes: List[Node]) -> None:
        ...
        assert all(
            node.metadata.node_id not in {n.metadata.node_id for n in store["nodes"]}
            for node in nodes
