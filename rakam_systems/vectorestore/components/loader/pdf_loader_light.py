...
class PdfLoaderLight(Loader):
    ...
    def __init__(
        ...
        self._scratch_folder_name = config.get('scratch_folder_name', 'scratch')
        ...
        self._image_path = config.get('image_path') or os.getenv('INGESTION_IMAGE_PATH', self.DEFAULT_IMAGE_PATH)
        
        # Chunker configuration
        self._max_tokens = config.get('max_tokens', self.DEFAULT_MAX_TOKENS)
        self._merge_peers = config.get('merge_peers', True)
        self._min_chunk_tokens = config.get('min_chunk_tokens', self.DEFAULT_MIN_CHUNK_TOKENS)
        self._filter_toc = config.get('filter_toc', True)
        
        # Initialize advanced chunker
        embed_model_id = config.get('embed_model_id', self.DEFAULT_EMBED_MODEL_ID)
        chunker_strategy = config.get('chunker_strategy', self.DEFAULT_CHUNKER_STRATEGY)
        ...
        logger.info(f"Initialized PdfLoaderLight with chunker_strategy={chunker_strategy}, page_chunks={self._page_chunks}, dpi={self._dpi}, image_path={self._image_path}")
    ...
    def load_as_text(
        ...
        if not self._is_pdf_file(source):
            raise ValueError(f"File is not a PDF: {source}. MIME type: {mimetypes.guess_type(source)[0]}")
        ...
        try:
            ...
            image_path = self._get_image_path(source) if self._write_images else None
            ...
            logger.info(f"PDF loaded as text in {elapsed:.2f}s: {len(md_text)} characters")
    ...
    def load_as_chunks(
        ...
        try:
            ...
            if self._save_markdown:
                if self._page_chunks and isinstance(md_result, list):
                    ...
                    for idx, page_md in enumerate(md_result):
                        self._save_markdown_file(source, page_md['text'], scratch_dir, page_num=idx+1)
            ...
            elapsed = time.time() - start_time
            logger.info(f"PDF processed in {elapsed:.2f}s: {len(text_chunks)} chunks")
    ...
    def load_as_vsfile(
        ...
        logger.info(f"Created VSFile with {len(nodes)} nodes from: {file_path}")
        return vsfile
    ...
    def _build_image_path_mapping(self, markdown_text: str, image_path: str) -> None:
        ...
        for alt_text, markdown_path in matches:
            ...
            if absolute_path.exists():
                ...
                logger.debug(f"Mapped image: {markdown_path} -> {absolute_path}")
            else:
                logger.warning(f"Image file not found: {absolute_path}")
        
        logger.info(f"Built image path mapping with {len(self._image_path_mapping)} images")
    ...
    def _save_markdown_file(
        ...
        try:
            ...
            if page_num is not None:
                md_filename = scratch_dir / f"{doc_filename}-page-{page_num}.md"
            else:
    ...
    def _extract_and_chunk_text(self, md_result: Union[str, List[Dict[str, Any]]]) -> List[str]:
        ...
        try:
            ...
            logger.warning(f"Failed to chunk document with AdvancedChunker: {e}")
            # Fall back to simple text extraction if advanced chunking fails
            logger.info("Falling back to simple text extraction")
            if isinstance(md_result, list):
                text_chunks = [page_dict.get('text', '') for page_dict in md_result if page_dict.get('text', '').strip()]
            else:
