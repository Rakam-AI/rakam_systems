...
### Why Use `from rakam_systems.ai_vectorstore.*`?
...
packages=find_packages(
    where="..",
    include=["rakam_systems.ai_vectorstore*", "rakam_systems.ai_core*"]
)
```
...
4. **Cross-module Ambiguity**: `..ai_core` is less clear than `rakam_systems.ai_core`
...
2. **Easy Testing**: Can mock `rakam_systems.ai_core` interfaces
...
The package name (`rakam-systems-ai-vectorstore`) is different from the import path (`rakam_systems.ai_vectorstore`), which is standard Python practice.
