...
agents:
  agent_name:
    ...
    # Fully qualified path to a Pydantic model class
    output_type: "myapp.models.MyOutputModel"
...
You can configure agents to return structured output in three ways:

#### Method 1: Pass Python Class Directly

Pass a Pydantic model class directly to `BaseAgent`:
...
class MovieRecommendation(BaseModel):
    """Structured output for movie recommendations."""
    title: str = Field(description="The movie title")
    year: int = Field(description="The release year")
    genre: str = Field(description="The primary genre")
    why_watch: str = Field(description="Why this movie is recommended")
    rating: float = Field(description="Rating out of 10")

agent = BaseAgent(
    name="movie_recommender",
    model="openai:gpt-4o",
    output_type=MovieRecommendation,  # <-- Pass the class directly
    system_prompt="You are a movie expert. Recommend one movie.",
)
```

#### Method 2: YAML with Inline Schema (No Python Class Needed!)

Define the output structure directly in the YAML configuration:
...
agents:
  movie_recommender:
    ...
    model_config:
      model: "openai:gpt-4o"
    prompt_config: "movie_expert"
    
    # Define output type inline - no Python class required!
    output_type:
      name: "MovieRecommendation"
      description: "Structured output for movie recommendations"
      fields:
        title:
          type: str
...
#### Method 3: YAML with Class Path Reference

Reference an existing Python class by its import path:
...
agents:
  movie_recommender:
    ...
    # Reference existing Python class by its full import path
    output_type: "myapp.models.MovieRecommendation"
```

#### Inline Output Type Field Options

Each field in the inline `output_type` supports these options:

| Option | Type | Description |
|--------|------|-------------|
| `type` | string | Field type: `str`, `int`, `float`, `bool`, `list`, `dict` |
| `description` | string | Description used in the LLM prompt |
| `default` | any | Default value for non-required fields |
| `default_factory` | string | For mutable defaults: `list` or `dict` |
| `required` | bool | Whether the field is required (default: true) |

#### Accessing Structured Output

Access the structured output the same way regardless of which method was used:

```python
result = await agent.arun("Recommend a sci-fi movie from the 90s", deps=deps)

# Access structured fields via result.output
print(result.output.title)      # "The Matrix"
print(result.output.year)       # 1999
print(result.output.genre)      # "Sci-Fi"
print(result.output.rating)     # 9.5
print(result.output.why_watch)  # "A groundbreaking film..."
```
...
| Method | Best For |
|--------|----------|
| **Method 1** (Python class direct) | IDE autocomplete, type checking, reusable models |
| **Method 2** (YAML inline) | Quick prototyping, config-driven agents, no Python needed |
| **Method 3** (YAML class path) | Sharing models across YAML configs, separation of concerns |
