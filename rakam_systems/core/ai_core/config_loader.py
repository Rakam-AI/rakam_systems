...
class ConfigurationLoader:
    ...
    def get_tool_registry(self, config: Optional[ConfigFileSchema] = None) -> ToolRegistry:
        ...
        if config is None:
            raise ValueError(
                "No configuration loaded. Call load_from_yaml or load_from_dict first.")
    ...
    def _register_tool(self, registry: ToolRegistry, tool_config: ToolConfigSchema) -> None:
        ...
        if tool_config.type == ToolMode.DIRECT:
            ...
            function = self._load_function(
                tool_config.module, tool_config.function)
            ...
            registry.register_direct_tool(
                ...
                json_schema=tool_config.json_schema or self._generate_schema(
                    function),
        ...
        for field_name, field_config in output_schema.fields.items():
            ...
            if field_config.default is not None:
                ...
                if field_config.default_factory.lower() in ('list', 'array', '[]'):
                    default_value = Field(
                        default_factory=list, description=field_config.description)
                    field_definitions[field_name] = (
                        python_type, default_value)
                    continue
                elif field_config.default_factory.lower() in ('dict', 'object', '{}'):
                    default_value = Field(
                        default_factory=dict, description=field_config.description)
                    field_definitions[field_name] = (
                        python_type, default_value)
                    continue
                else:
    ...
    def _resolve_output_type(self, output_type_config: Optional[str | OutputTypeSchema]) -> Optional[Type]:
        ...
        if isinstance(output_type_config, str):
            ...
            raise ValueError(
                f"Invalid output_type configuration: {type(output_type_config)}")
    ...
    def resolve_prompt_config(
        ...
        if prompt_ref not in config.prompts:
            raise ValueError(
                f"Prompt '{prompt_ref}' not found in configuration")
        ...
        for ref in tool_refs:
            if isinstance(ref, ToolConfigSchema):
                ...
                if ref not in config.tools:
                    raise ValueError(
                        f"Tool '{ref}' not found in configuration")
    ...
    def create_agent(
        ...
        if agent_class is None:
            from rakam_systems.agent.components import BaseAgent
            agent_class = BaseAgent

        # Resolve prompt config
        prompt_config = self.resolve_prompt_config(
            agent_config.prompt_config, config)
